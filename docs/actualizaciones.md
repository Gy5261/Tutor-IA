...
Para la versión 4.1.4, descarga el instalador y manifiesto de [la misma release](https://github.com/Gy5261/Tutor-IA/releases/tag/v4.1.4). Desde esa carpeta:

```powershell
$manifest = Get-Content -Raw -LiteralPath .\release-manifest.json | ConvertFrom-Json
$installer = Get-Item -LiteralPath .\Tutor-IA-Setup-4.1.4.exe
...
El manifiesto publicado de 4.1.4 declara 137.004.005 bytes y estado `NotSigned`. Su SHA-256 coincide con el digest del asset publicado en GitHub. Eso acredita coherencia del artefacto con el manifiesto, no una firma del editor.
