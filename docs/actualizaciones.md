...
| Archivo | Uso |
| --- | --- |
| `Tutor-IA-Setup-6.1.10.exe` | Instalador |
...
Descarga el instalador y el manifiesto de la [misma release](https://github.com/Gy5261/Tutor-IA/releases/tag/v6.1.10). En PowerShell, desde esa carpeta:

```powershell
$manifest = Get-Content -Raw -LiteralPath .\release-manifest.json | ConvertFrom-Json
$installer = Get-Item -LiteralPath .\Tutor-IA-Setup-6.1.10.exe
...
Los hashes de 6.1.10 también están en [checksums.txt](../checksums.txt). Si el manifiesto indica `NotSigned`, el instalador no tiene firma Authenticode; un hash correcto no equivale a firma digital.
